            ...
            <thead>
                <tr>
                    ...
                    if ($view == 'podsumowanie_sprzedazy') {
                        echo "<th>Nazwa Produktu</th><th>Liczba Zamówień</th><th>Łączność Sprzedaży</th>";
                    } elseif ($view == 'produkty_z_kategoria') {
                        echo "<th>ID Produktu</th><th>Nazwa Produktu</th><th>Cena Produktu</th><th>Rozmiar Produktu</th><th>Kategoria</th>";
                    } elseif ($view == 'szczegoly_zamowienia') {
                        echo "<th>ID Zamówienia</th><th>Imię Klienta</th><th>Nazwisko Klienta</th><th>Miejscowość</th><th>Nazwa Produktu</th><th>Cena Produktu</th><th>Data Zamówienia</th><th>Status Zamówienia</th>";
            ...
            <tbody>
                <?php foreach ($data as $row): ?>
                    <tr>
                        <?php
                        if ($view == 'podsumowanie_sprzedazy') {
                            echo "<td>{$row['Nazwa_Produktu']}</td><td>{$row['Liczba_Zamowien']}</td><td>{$row['LacznoscSprzedazy']}</td>";
                        } elseif ($view == 'produkty_z_kategoria') {
                            echo "<td>{$row['ID_Produktu']}</td><td>{$row['Nazwa_Produktu']}</td><td>{$row['Cena_Produktu']}</td><td>{$row['Rozmiar_Produktu']}</td><td>{$row['Kategoria']}</td>";
                        } elseif ($view == 'szczegoly_zamowienia') {
                            echo "<td>{$row['ID_Zamowienia']}</td><td>{$row['Imie_Klienta']}</td><td>{$row['Nazwisko_Klienta']}</td><td>{$row['Miejscowosc']}</td><td>{$row['Nazwa_Produktu']}</td><td>{$row['Cena_Produktu']}</td><td>{$row['Data_Zamowienia']}</td><td>{$row['Status_Zamowienia']}</td>";
                        } elseif ($view == 'szczegoly_wysylek') {
